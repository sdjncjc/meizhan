$(function() {
	...
	var changeAddress = function(key, area_id, city_id, freight_hash) {
		$.ajax({//保存收货地址
			type:'post',
			...
			success:function(result){
				...
				if (data.no_send_tpl_ids.length > 0) {
					$.each(data.no_send_tpl_ids, function(k,v){
						$('#goods_tpl_' + v).html('（无货）');
					});
					$('#buy_step2').addClass('no-submit');
				} else {
					$('.goods_tpl').html('');
					$('#buy_step2').removeClass('no-submit');
				}
	...
	$.ajax({//提交订单信息
		...
		success:function(result){
			...
			$.each(data.store_cart_list,function(k,v){//循环店铺
				...
				i++;
				htmldata+='<p class="buys-yt-tlt">店铺名称：'+v.store_name+'<span data-store_id="'+k+'" class="store-cod-supported" style="display:none;">（该店铺不支持选定收货地址的货到付款）</span><span id="store_zk_' + k + '" class="store-zk-supported" style="display:none;"></span></p>';
						$.each(v.goods_list,function(k1,v1){//循环商品列表
							...
							htmldata+='<div class="clearfix">'
												...
												+'<div class="buys1-pdlcnt">'
													...
													+'<p>数量：<span>'+v1.goods_num+'</span><span class="goods_tpl" id="goods_tpl_' + v1.transport_id + '"></span></p>'
												+'</div>'
											+'</div>'
										+'</div>';
						});
			...
			// 平台红包
			if (data.rpt_list.length > 0) {
				$('.pre-deposit-wp').show();
				$('#wrapper-userrpt').show();
				var _htmlRpt = '';
				$.each(data.rpt_list, function(k, v) {
					_htmlRpt += '<option data-limit="' + v.rpacket_limit + '" data-price="' + v.rpacket_price + '" value="' + v.rpacket_t_id + '|' + v.rpacket_price + '" >' + v.desc + '</option>';
				});
				$('#rpt').append(_htmlRpt);
			}
			
			// 会员等级折扣
			$.each(data.zk_list, function(k, v) {
				$('#store_zk_' + k).show().html('（' + v + '）');
			});
...
*/
			...
			changeAddress(key, area_id, city_id, freight_hash);
			...
			$('select[name=rpt]').change(function(){
				...
				var _rptLimit = parseFloat($(this).find('option').eq(_index).attr('data-limit'));
				var _rptPrice = parseFloat($(this).find('option').eq(_index).attr('data-price'));
				var _totalPrice = parseFloat($('#total_price').html());

				if (_totalPrice < _rptLimit) {	
					// 重置红包
					resetRpt();
					$('.rpt_error_tip').show().html('这个红包不可以使用');
				} else {
					$('#total_price').html(p2f(_totalPrice - _rptPrice));
				}
			});
    ...
    $(".save-address").click(function (){//更换收获地址
        ...
        if(address_id>0){//变更地址
			...
			if($.sValid()){
				...
				{ 
                    $(".error-tips").html('请填写正确的手机号').show();
				    return false; 
				} 
                ...
                if ($('#area_select').find('select').last().val() == '') {
                    $(".error-tips").html('请选择到最后一级地区').show();
                    return false;
                }
				var area_info = '';
				if (typeof($('#area_select > select').eq(0)[0]) != 'undefined') {
					area_info += $('#area_select > select').eq(0)[0].options[$('#area_select > select').eq(0)[0].selectedIndex].innerHTML;
				}
				if (typeof($('#area_select > select').eq(1)[0]) != 'undefined') {
					area_info += ' ' + $('#area_select > select').eq(1)[0].options[$('#area_select > select').eq(1)[0].selectedIndex].innerHTML;
				}
				if (typeof($('#area_select > select').eq(2)[0]) != 'undefined') {
					area_info += ' ' + $('#area_select > select').eq(2)[0].options[$('#area_select > select').eq(2)[0].selectedIndex].innerHTML;
				}
				if (typeof($('#area_select > select').eq(3)[0]) != 'undefined') {
					area_info += ' ' + $('#area_select > select').eq(3)[0].options[$('#area_select > select').eq(3)[0].selectedIndex].innerHTML;
				}
...
*/
		...
		if(verified){
			$.ajax({//验证实名认证
				type:'post',
				url:ApiUrl+'/index.php?act=member_buy&op=check_verified',
				data:{key:key},
				dataType:'json',
				success:function(result){
					if(result.datas == 'true'){
						...
						$.sDialog({
							...
							okBtnText:"立即认证",
							okFn:function (){
								window.open(WapSiteUrl+"/tmpl/member/verified.html")
							}
						});
					}
				}
    ...
    $(".buys1-edit-group").click(function(){
        var thisPrarent = $(this).parents(".buys1-group-cnt");
        hideDetail(thisPrarent);
    });
    $(".no-group").click(function (){
        var thisPrarent = $(this).parents(".buys1-group-cnt");
        showDetial(thisPrarent);
    });
    $(".save-group").click(function (){//保存发票信息
		...
		if(group_pass == ''){
			...
			$.ajax({
            	type:'post',
            	url:ApiUrl+'/index.php?act=group&op=group_add',
            	data:{key:key,group_pass:group_pass},
            	dataType:'json',
            	success:function(result){
            		if(result.datas.msg == 'true') {
						get_group();
					} else {
	...
	function get_group(){
		$.ajax({
			type:'post',
			url:ApiUrl+'/index.php?act=group&op=index',
			data:{key:key},
			dataType:'json',
			success:function(result){
				if(result.datas.group_id > 0){
					$('.buys1-edit-group').hide();
					$(".buys1-group-cnt").find(".buys1-hide-list").addClass("hide");
					$(".buys1-group-cnt").find(".buys1-hide-detail").removeClass("hide");
					$('.buys1-group-cnt .buys1-hide-detail').html('<li class="clearfix">专属客服：'+result.datas.group.group_name+'</li><li class="clearfix">微信号：'+result.datas.group.group_wx_name+'</li><li class="clearfix">手机号：'+result.datas.group.group_wx_mobile+'</li>');
				}else{
					$('.buys1-group-cnt .iadd-cnt').html('推荐客服：'+result.datas.group.group_name+'<br>微信号：'+result.datas.group.group_wx_name+'<br>手机号：'+result.datas.group.group_wx_mobile);
				}
			}
		});
	}
